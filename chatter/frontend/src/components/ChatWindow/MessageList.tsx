...
import { useUserAvatar } from '../../hooks/useUser';
  ...
  ({ avatar, contactId, ownerId }, ref) => {
    ...
    useImperativeHandle(ref, () => ({
      addMessage(newMessage: Message) {
        addMessage(newMessage.senderId, newMessage.recipientId, newMessage.message);
        scrollToBottom();
      },
    }));
    ...
    const getItemSize = useCallback(
      (index: number) => {
        ...
        if (!message) {
          return 80; // Default height if message is undefined
        }
        const baseHeight = 80;
        const extraHeight = Math.ceil(message.message.length / 2);
